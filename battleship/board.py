...
class Board:
    ...
    def __init__(self, ships=None, size=(10,10), 
        ...
        # Dict storing the number of ships per length
    ...
    def is_attacked_at(self, cell):
        ...
        - if there is a ship at that position -> the ship is damaged at that coordinate
        ...
        Returns:
            tuple : (is_ship_hit, has_ship_sunk) where
                - is_ship_hit is True if and only if the cell is occupied by a ship (False otherwise)
                - has_ship_sunk is True if and only if the attack made the ship sink (False otherwise)
        """
    ...
    def _array_to_str(self, array_board):
        ...
        array_first_line = [chr(code + CellConverter.UPPERCASE_OFFSET) for code in range(1, self.width + 1)]
